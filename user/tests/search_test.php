...
class user_search_testcase extends advanced_testcase {
    ...
    public function test_users_document() {
        ...
        $this->assertEquals(fullname($user), $doc->get('title'));
        $this->assertEquals($user->email, $doc->get('content'));
        $this->assertEquals(content_to_text($user->firstname.' '.$user->middlename.' '.$user->lastname.' ( '.$user->username.' )', false), $doc->get('description1'));        
    }
